...
AllreducePlugin::AllreducePlugin(std::set<int> group, nvinfer1::DataType type,
                                 AllReduceStrategyType strategy, AllReduceStrategyConfig config,
                                 int32_t counter)
    : mGroup(std::move(group)), mType(type), mStrategy(strategy), mConfig(config),
      mCounter(counter) {}
...
nvinfer1::DimsExprs
AllreducePlugin::getOutputDimensions(int outputIndex, nvinfer1::DimsExprs const *inputs,
                                     int nbInputs, nvinfer1::IExprBuilder &exprBuilder) noexcept {
  return inputs[0];
}

bool AllreducePlugin::supportsFormatCombination(int pos, nvinfer1::PluginTensorDesc const *inOut,
...
AllReduceStrategyType AllreducePlugin::selectImplementation(size_t messageSize, int worldSize,
...
int AllreducePlugin::enqueue(nvinfer1::PluginTensorDesc const *inputDesc,
  ...
  if (runtimeStrategy == AllReduceStrategyType::NCCL) {
    auto *commMap = getCommMapManager();
    NCCLCHECK(ncclAllReduce(inputs[0], outputs[0], size, (*getDtypeMap())[mType], ncclSum,
                            commMap->get(mGroup), stream));
  } else {
...
nvinfer1::DataType AllreducePlugin::getOutputDataType(int index,
...
char const *AllreducePlugin::getPluginType() const noexcept { return ALLREDUCE_PLUGIN_NAME; }

char const *AllreducePlugin::getPluginVersion() const noexcept { return ALLREDUCE_PLUGIN_VERSION; }

int AllreducePlugin::getNbOutputs() const noexcept { return 1; }
...
bool AllreducePlugin::isCustomAllReduceSuported(int ranks_per_node) const noexcept {
  ...
  return isCudaVersionSupported && (ranks_per_node % 2 == 0) &&
         (ranks_per_node <= kernels::MAX_RANKS_PER_NODE) && (ranks_per_node > 0);
}

int AllreducePlugin::initialize() noexcept {
  if (isBuilding() || mStrategy == AllReduceStrategyType::ONESHOT ||
...
void AllreducePlugin::terminate() noexcept {
  if (mStrategy == AllReduceStrategyType::NCCL || mStrategy == AllReduceStrategyType::AUTO) {
    ...
    if (isBuilding() || commMap->get(mGroup) == nullptr) {
      return;
    }
    commMap->release(mGroup);
  }
}

size_t AllreducePlugin::getSerializationSize() const noexcept {
  return sizeof(int) * mGroup.size() + sizeof(mType) + sizeof(mStrategy) + sizeof(mConfig) +
         sizeof(mCounter);
}
...
char const *AllreducePluginCreator::getPluginName() const noexcept { return ALLREDUCE_PLUGIN_NAME; }

char const *AllreducePluginCreator::getPluginVersion() const noexcept {
  return ALLREDUCE_PLUGIN_VERSION;
}

PluginFieldCollection const *AllreducePluginCreator::getFieldNames() noexcept { return &mFC; }

IPluginV2 *AllreducePluginCreator::createPlugin(char const *name,
  ...
  for (int i = 0; i < fc->nbFields; ++i) {
    ...
    if (!strcmp(attrName, "group")) {
      ...
      TLLM_CHECK(fields[i].type == PluginFieldType::kINT32);
      type = static_cast<nvinfer1::DataType>(
          *(static_cast<nvinfer1::DataType const *>(fields[i].data)));
...
IPluginV2 *AllreducePluginCreator::deserializePlugin(char const *name, void const *serialData,
