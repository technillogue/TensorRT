...
public:
  AllreducePlugin(std::set<int> group, nvinfer1::DataType type,
                  kernels::AllReduceStrategyType strategy, kernels::AllReduceStrategyConfig config,
                  int32_t counter);
  ...
  nvinfer1::DimsExprs getOutputDimensions(int outputIndex, nvinfer1::DimsExprs const *inputs,
                                          int nbInputs,
...
private:
  static kernels::AllReduceStrategyType selectImplementation(size_t messageSize, int worldSize,
