...
AllgatherPlugin::AllgatherPlugin(std::set<int> group, nvinfer1::DataType type)
    : mGroup(std::move(group)), mType(type) {}
...
nvinfer1::DimsExprs
AllgatherPlugin::getOutputDimensions(int outputIndex, nvinfer1::DimsExprs const *inputs,
...
bool AllgatherPlugin::supportsFormatCombination(int pos, nvinfer1::PluginTensorDesc const *inOut,
...
int AllgatherPlugin::enqueue(nvinfer1::PluginTensorDesc const *inputDesc,
  ...
  NCCLCHECK(ncclAllGather(inputs[0], outputs[0], size, (*getDtypeMap())[inputDesc[0].type],
...
nvinfer1::DataType AllgatherPlugin::getOutputDataType(int index,
...
char const *AllgatherPlugin::getPluginType() const noexcept { return ALLGATHER_PLUGIN_NAME; }

char const *AllgatherPlugin::getPluginVersion() const noexcept { return ALLGATHER_PLUGIN_VERSION; }
...
char const *AllgatherPluginCreator::getPluginName() const noexcept { return ALLGATHER_PLUGIN_NAME; }

char const *AllgatherPluginCreator::getPluginVersion() const noexcept {
  return ALLGATHER_PLUGIN_VERSION;
}

PluginFieldCollection const *AllgatherPluginCreator::getFieldNames() noexcept { return &mFC; }

IPluginV2 *AllgatherPluginCreator::createPlugin(char const *name,
...
IPluginV2 *AllgatherPluginCreator::deserializePlugin(char const *name, void const *serialData,
