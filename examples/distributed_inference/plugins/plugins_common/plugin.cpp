 ...
 */
#include "plugins_common/plugin.h"
...
void CommMapManager::init(std::set<int> const &group) {
  std::lock_guard<std::mutex> lock(mMutex);
  if (!(refMap.find(group) != refMap.end() && refMap[group] > 0)) {
    initCommMap(group);
    refMap[group] = 0;
  }
  ++refMap[group];
...
ncclComm_t CommMapManager::get(std::set<int> const &group) {
  std::lock_guard<std::mutex> lock(mMutex);
  return commMap[group];
}

ncclComm_t CommMapManager::release(std::set<int> const &group) {
  std::lock_guard<std::mutex> lock(mMutex);
  // assert(refMap.find(group) != refMap.end() && refMap[group] > 0);
  --refMap[group];
  if (refMap[group] == 0) {
    NCCLCHECK(ncclCommDestroy(commMap[group]));
    commMap[group] = nullptr;
  }
}

void CommMapManager::initCommMap(std::set<int> const &group) {
...
CommMapManager *getCommMapManager() {
  static CommMapManager commMap{};
  return &commMap;
}
...
// resources.
template <typename T> class PerCudaCtxSingletonCreator {
public:
  ...
  // So when T is released, but we still have observer weak_ptr in mObservers, the T mem block can
  // be released. creator itself must not own CUDA resources. Only the object it creates can.
  PerCudaCtxSingletonCreator(CreatorFunc creator, DeleterFunc deleter)
      : mCreator{std::move(creator)}, mDeleter{std::move(deleter)} {}
  ...
  std::shared_ptr<T> operator()() {
    ...
    if (result == nullptr) {
      ...
      result = std::shared_ptr<T>{
          mCreator().release(), [this, ctx](T *obj) {
            if (obj == nullptr) {
              return;
            }
            mDeleter(obj);

            // Clears observer to avoid growth of mObservers, in case users creates/destroys cuda
            // contexts frequently.
            std::shared_ptr<T> observedObjHolder; // Delay destroy to avoid dead lock.
            std::lock_guard<std::mutex> lk{mMutex};
            // Must check observer again because another thread may created new instance for this
            // ctx just before we lock mMutex. We can't infer that the observer is stale from the
            // fact that obj is destroyed, because shared_ptr ref-count checking and observer
            // removing are not in one atomic operation, and the observer may be changed to observe
            // another instance.
...
template <typename T> nvinfer1::PluginFieldType toFieldType();
#define SPECIALIZE_TO_FIELD_TYPE(T, type)                                                          \
  template <> nvinfer1::PluginFieldType toFieldType<T>() { return nvinfer1::PluginFieldType::type; }
...
template <typename T> std::optional<T> PluginFieldParser::getScalar(std::string_view const &name) {
