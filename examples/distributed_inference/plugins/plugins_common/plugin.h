...
#include "plugins_common/checkMacrosPlugin.h"
...
template <typename T> void write(char *&buffer, T const &val) {
  std::memcpy(buffer, &val, sizeof(T));
  buffer += sizeof(T);
}

// Read values from buffer
template <typename T> void read(char const *&buffer, T &val) {
...
#endif
  default:
    TLLM_THROW("Not supported data type for cuBLAS");
  }
}

// Like std::unique_ptr, but does not prevent generation of default copy constructor when used as
// class members. The copy constructor produces nullptr. So the plugin default copy constructor will
// not really copy this, and your clone() implementation is responsible for initializing such data
// members. With this we can simplify clone() implementation when there are many data members
// including at least one unique_ptr.
...
public:
  ...
  UniqPtrWNullCopy(UniqPtrWNullCopy const &) : std::unique_ptr<T, Del>::unique_ptr{} {}
};
...
class CommMapManager {
public:
  void init(std::set<int> const &group);
  ncclComm_t get(std::set<int> const &group);
  ncclComm_t release(std::set<int> const &group);

private:
  void initCommMap(std::set<int> const &group);

  std::map<std::set<int>, int> refMap;
  std::map<std::set<int>, ncclComm_t> commMap;
...
public:
  ...
  // delete to remind accidental mis-use (copy) which may result in false-alarm warnings about
  // unused fields.
  ...
  ~PluginFieldParser();
  template <typename T> std::optional<T> getScalar(std::string_view const &name);

private:
  nvinfer1::PluginField const *mFields;

  struct Record {
    Record(int32_t idx) : index{idx} {}
