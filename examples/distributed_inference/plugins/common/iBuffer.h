...
#ifdef ENABLE_BF16
template <> struct DataTypeTraits<nvinfer1::DataType::kBF16> {
...
#ifdef ENABLE_FP8
template <> struct DataTypeTraits<nvinfer1::DataType::kFP8> {
...
public:
  ...
  [[nodiscard]] constexpr nvinfer1::DataType getDataType() const noexcept { return mDataType; }
...
public:
  ...
  //!
  [[nodiscard]] virtual std::size_t getSizeInBytes() const { return toBytes(getSize()); }
  ...
  //!
  //! \brief Creates a sliced view on the underlying `buffer`. The view will have the same data type
  //! as `buffer`.
  //!
  ...
  //!
  //! \brief Wraps the given `data` in an `IBuffer`. The `IBuffer` will not own the underlying
  //! `data` and cannot be resized beyond `capacity`.
  //!
  ...
  template <typename T> static UniquePtr wrap(T *data, std::size_t size, std::size_t capacity) {
    return wrap(data, TRTDataType<T>::value, size, capacity);
  }

  template <typename T> static UniquePtr wrap(T *data, std::size_t size) {
    return wrap<T>(data, size, size);
  }

  template <typename T> static UniquePtr wrap(std::vector<T> &v) {
...
public:
  ...
  BufferRange(T *data, size_type size) : Base{data, size} {}

  explicit BufferRange(IBuffer &buffer) : BufferRange(bufferCast<T>(buffer), buffer.getSize()) {}
};
