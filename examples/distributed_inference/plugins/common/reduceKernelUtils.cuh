...
template <int Bytes> __device__ inline void copy(void const *local, void *data) {
...
/* Calculate the maximum of all elements in a block */
template <typename T> __inline__ __device__ T blockAllReduceMax(T val) {
  static __shared__ T shared[32];
  int lane = threadIdx.x & 0x1f; // in-warp idx
  int wid = threadIdx.x >> 5;    // warp idx
...
template <typename T, int NUM> __inline__ __device__ T warpReduceSumV2(T *val) {
#pragma unroll
  for (int i = 0; i < NUM; i++) {
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1)
...
template <typename T, int NUM> __inline__ __device__ T blockReduceSumV2(T *val) {
...
template <typename T, int NUM> __inline__ __device__ T warpReduceMaxV2(T *val) {
#pragma unroll
  for (int i = 0; i < NUM; i++) {
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1)
...
template <typename T, int NUM> __inline__ __device__ T blockReduceMaxV2(T *val) {
...
template <typename T, int MAX_K> struct TopK {
...
template <typename T> struct TopK_2 {
  int p = -1;
...
template <typename T> __device__ __forceinline__ T clamp_inf_for_half(float const input) {
  return input;
}

template <> __device__ __forceinline__ half clamp_inf_for_half(float const input) {
