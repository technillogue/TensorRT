...
public:
  ...
  //!
  void squeeze(SizeType dim) { reshape(squeeze(getShape(), dim)); }

  //!
  //! \brief Adds a *unit* dimension at the specified position
  //!
  void unsqueeze(SizeType dim) { reshape(unsqueeze(getShape(), dim)); }

  //!
  //! \brief Creates a sliced view on the underlying `tensor`. The view will have the same data type
  //! as `tensor`.
  //!
  ...
  template <typename TConstPtr,
            std::enable_if_t<std::is_const_v<PointerElementType<TConstPtr>>, int> = 0>
  ...
  //!
  //! \brief Wraps the given `data` in an `ITensor`. The `ITensor` will not own the underlying
  //! `data` and cannot be reshaped beyond `capacity`.
  //!
  ...
  template <typename T> static UniquePtr wrap(T *data, Shape const &shape, std::size_t capacity) {
    return wrap(data, TRTDataType<T>::value, shape, capacity);
  }

  template <typename T> static UniquePtr wrap(T *data, Shape const &shape) {
    return wrap<T>(data, shape, volumeNonNegative(shape));
  }

  template <typename T> static UniquePtr wrap(std::vector<T> &v, Shape const &shape) {
  ...
  //! \brief A convenience function to compare shapes.
  //!
  template <typename T> static bool shapeEquals(Shape const &lhs, T const *dims, SizeType count) {
    return lhs.nbDims == count && std::equal(lhs.d, lhs.d + lhs.nbDims, dims);
  }

  bool shapeEquals(Shape const &other) const { return shapeEquals(getShape(), other); }

  bool shapeEquals(std::initializer_list<SizeType> const &other) const {
    return shapeEquals(getShape(), other.begin(), other.size());
  }

  template <typename T> bool shapeEquals(T const *dims, SizeType count) const {
...
protected:
  ...
  static DimType castSize(size_t newSize) {
    TLLM_CHECK_WITH_INFO(newSize <= std::numeric_limits<DimType>::max(),
