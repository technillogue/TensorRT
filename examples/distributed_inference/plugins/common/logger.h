...
#if defined(_MSC_VER)
  template <typename... Args> void log(Level level, char const *format, Args const &...args);
...
template <typename... Args>
void Logger::log(const Logger::Level level, int const rank, char const *format,
                 Args const &...args) {
  if (level_ <= level) {
...
#define TLLM_LOG_EXCEPTION(ex, ...)                                                                \
  tensorrt_llm::common::Logger::getLogger()->log(ex, ##__VA_ARGS__)
} // namespace tensorrt_llm::common
