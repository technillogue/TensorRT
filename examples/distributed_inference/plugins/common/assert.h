...
#define TLLM_CHECK_WITH_INFO(val, info, ...)                                                       \
  do {                                                                                             \
    TLLM_LIKELY(static_cast<bool>(val))                                                            \
    ? ((void)0)                                                                                    \
    : tensorrt_llm::common::throwRuntimeError(__FILE__, __LINE__,                                  \
                                              tensorrt_llm::common::fmtstr(info, ##__VA_ARGS__));  \
  } while (0)
...
#define TLLM_WRAP(ex)                                                                              \
  NEW_TLLM_EXCEPTION("%s: %s",                                                                     \
                     tensorrt_llm::common::TllmException::demangle(typeid(ex).name()).c_str(),     \
                     ex.what())
