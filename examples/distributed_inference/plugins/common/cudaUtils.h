...
/* **************************** debug tools ********************************* */
static char const *_cudaGetErrorEnum(cudaError_t error) { return cudaGetErrorString(error); }
...
#ifdef ENABLE_BF16
template <> struct CudaDataType<__nv_bfloat16> {
...
inline std::tuple<size_t, size_t> getDeviceMemoryInfo(bool const useUvm) {
  if (useUvm) {
    ...
    TLLM_LOG_INFO("Using UVM based system memory for KV cache, total memory %0.2f GB, available "
                  "memory %0.2f GB",
    ...
    TLLM_LOG_DEBUG(
        "Using GPU memory for KV cache, total memory %0.2f GB, available memory %0.2f GB",
...
template <typename T1, typename T2> inline size_t divUp(const T1 &a, const T2 &n) {
...
void printAbsMean(T const *buf, uint64_t size, cudaStream_t stream, std::string name = "") {
  ...
  TLLM_LOG_INFO("%20s size: %u, abs mean: %f, abs sum: %f, abs max: %f, find inf: %s", name.c_str(),
                size, sum / size, sum, max_val, find_inf ? "true" : "false");
  delete[] h_tmp;
  cudaDeviceSynchronize();
  check_cuda_error(cudaGetLastError());
}

template <typename T> void printToStream(T const *result, int const size, FILE *strm) {
...
template <typename T> void printToScreen(T const *result, int const size) {
