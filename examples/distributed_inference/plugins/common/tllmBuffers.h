...
#include "runtime/memoryCounters.h"
...
// CRTP base class
template <typename TDerived, MemoryType memoryType, bool count = true> class BaseAllocator {
public:
...
public:
  ...
  explicit MemoryPool(SizeType chunkSize = kInitialChunkSize, Allocator allocator = Allocator{})
      : mChunkSize(chunkSize), mAllocator{allocator} {}
  ...
  [[nodiscard]] SizeType getUsedSize() const {
    ...
    return std::accumulate(
        mMemorySegments.cbegin(), mMemorySegments.cend(), SizeType{0},
  ...
  [[nodiscard]] SizeType getReservedSize() const {
    std::lock_guard<std::mutex> lock(mLock);
    return std::accumulate(
        mAllocatedChunks.cbegin(), mAllocatedChunks.cend(), SizeType{0},
        [](SizeType sum, auto const &chunk) { return sum + std::get<1>(chunk); });
  }

  class MemorySegment {
  public:
    MemorySegment(PointerType basePointer, SizeType size, SizeType offset = 0,
                  PointerType tag = nullptr)
        : basePointer{basePointer}, size{size}, offset{offset}, tag{tag} {}
...
template <typename TAllocator>
void MemoryPool<TAllocator>::allocateImpl(MemoryPool::PointerType *ptr,
                                          MemoryPool::SizeType requestedSize) {
  std::lock_guard<std::mutex> lock(mLock);
  ...
  TLLM_LOG_DEBUG("MemoryPool: Requested to reserve %zu B (%zu B aligned)", requestedSize,
                 alignedRequest);

  // Finds first free segment providing sufficient space
  auto it = std::find_if(
      mMemorySegments.begin(), mMemorySegments.end(),
...
void MemoryPool<TAllocator>::deallocateImpl(PointerType tag, SizeType n) {
  ...
  TLLM_CHECK_WITH_INFO(it != mMemorySegments.end(),
                       "MemoryPool free: Requested tag %p could not be found", tag);
...
template <typename TAllocator> void MemoryPool<TAllocator>::logSegments() const {
  std::lock_guard<std::mutex> lock(mLock);
...
template <typename TAllocator>
class PoolAllocator
    : public BaseAllocator<PoolAllocator<TAllocator>, TAllocator::kMemoryType, false> {
...
//!
template <typename TAllocator> class GenericBuffer : virtual public IBuffer {
public:
  ...
  //!
  void *data() override { return TLLM_LIKELY(mSize > 0) ? mBuffer : nullptr; }
  ...
  //!
  [[nodiscard]] MemoryType getMemoryType() const override { return mAllocator.getMemoryType(); }
...
protected:
  explicit GenericBuffer(std::size_t size, std::size_t capacity, nvinfer1::DataType type,
...
template <typename T> typename std::make_unsigned<T>::type nonNegative(T value) {
...
public:
  ...
  explicit GenericTensor(nvinfer1::Dims const &dims, std::size_t capacity, nvinfer1::DataType type,
                         TAllocator allocator = {})
      : Base{nonNegative(volume(dims)), capacity, type, std::move(allocator)}, mDims{dims} {}

  GenericTensor(GenericTensor &&tensor) noexcept : Base{std::move(tensor)}, mDims{tensor.dims} {
