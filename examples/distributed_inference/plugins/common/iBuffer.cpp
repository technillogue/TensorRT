...
#include "common/iTensor.h"
// #include "common/tllmBuffers.h"
...
IBuffer::UniquePtr IBuffer::slice(IBuffer::SharedPtr buffer, std::size_t offset, std::size_t size) {
  return nullptr; // std::make_unique<BufferView>(std::move(buffer), offset, size);
}

// IBuffer::UniquePtr IBuffer::wrap(void* data, nvinfer1::DataType type, std::size_t size,
// std::size_t capacity)
// {
...
//             new GenericBuffer<CpuBorrowingAllocator>(capacity, type, CpuBorrowingAllocator(data,
//             capacityInBytes)));
//         break;
//     case MemoryType::kGPU:
//         result.reset( // NOLINT(modernize-make-unique)
//             new GenericBuffer<GpuBorrowingAllocator>(capacity, type, GpuBorrowingAllocator(data,
//             capacityInBytes)));
//         break;
...
std::ostream &tensorrt_llm::runtime::operator<<(std::ostream &output, IBuffer const &buffer) {
  ...
  auto tensor = ITensor::wrap(data, buffer.getDataType(),
                              ITensor::makeShape({static_cast<SizeType>(buffer.getSize())}),
                              buffer.getCapacity());
